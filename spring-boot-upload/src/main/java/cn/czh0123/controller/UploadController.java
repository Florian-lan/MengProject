...
public class UploadController {
    ...
    // // 多张
    // @PostMapping(value = "/upload", produces = MediaType.IMAGE_JPEG_VALUE)
    // public ResponseEntity<ArrayList<byte[]>> upload_s(@RequestParam("image") MultipartFile file) throws IOException {
    //     // file校验
    //     if (file.isEmpty()) {
    //         return ResponseEntity.badRequest().body(null);
    //     }
    ...
    //     String filePath2 = pre + "/src/main/resources/static/images/test1.jpeg";
    ...
    //     byte[] imageBytes1 = Files.readAllBytes(newFile1.toPath());
    //     byte[] imageBytes2 = Files.readAllBytes(newFile2.toPath());

    //     ArrayList<byte[]> imageBytes = new ArrayList<>();
    //     imageBytes.add(imageBytes1);
    //     imageBytes.add(imageBytes2);

    //     HttpHeaders headers = new HttpHeaders();
    //     headers.setContentType(MediaType.IMAGE_JPEG);
    //     headers.setContentLength(imageBytes.size());
    //     return new ResponseEntity<>(imageBytes, headers, HttpStatus.OK);
    // }
}
